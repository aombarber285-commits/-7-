...
def analyze(symbol, candles_1m):
    ...
    # Early = เฝ้ารอเท่านั้น ไม่เปิดไม้
    early = (
...
def send_confirmed(data):
    ...
    # คู่เดิมมีไม้ค้างอยู่ = ห้ามเปิดเพิ่ม
...
def evaluate_trades():
    ...
    for key, trade in list(PENDING_TRADES.items()):
        ...
        # ต้องใช้ candle ที่ timestamp >= expiry
        candidates = [
            x for x in candles
...
path = Path("/mnt/data/tradeify_v6_1in3_real_tracker.py")
