...
def check_time_filters(now_utc):
    session = get_current_session(now_utc)
    if session not in TRADE_SESSIONS:
        return False, f"นอกเซสชัน ({session})"
    if USE_NEWS_FILTER and now_utc.hour in [12, 13, 14] and now_utc.minute in [0, 15, 30, 45]:
...
def run_strategy_gbp_jpy():
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚡ เริ่มระบบวิเคราะห์ (8 Indicators + แจ้งเตือน Discord 2 นาที): {name}\n")
        ...
        try:
            ...
            if now.minute != last_processed_minute:
                ...
                if df is not None:
                    ...
                    entry_time = (now + timedelta(minutes=2)).replace(second=0, microsecond=0)
                    action_icon = "🟢 ซื้อขึ้น (CALL)" if action == "CALL" else "🔴 ซื้อลง (PUT)"
                    
                    print(f"\n==================================================")
                    print(f"[{now.strftime('%H:%M:%S')}] ⚠️ [แจ้งเตือนเตรียมตัวล่วงหน้า 2 นาที] {name}")
                    ...
                    alert_msg = (
                        ...
                        f"⏰ **เวลาเข้าซื้อจริง:** `{entry_time.strftime('%H:%M:%S')}` น. (ล่วงหน้า 2 นาที)\n"
...
# ==========================================
# 💎 HELPER & CORE LOGIC FOR สูตรที่7 A+
# ==========================================
...
def pair_worker_strategy7(pair):
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚡ เริ่มระบบเข้าซื้อ [สูตรที่7 A+] {name} (โหมดแจ้งเตือนล่วงหน้า 2 นาที)...\n")
        ...
        try:
            ...
            if now.minute != last_checked_minute:
                ...
                if action:
                    ...
                    print(f"⏰ เช่น ตอนนี้ {now_str} น. -> ทำกำไร {target_str} น. {action_text}")
                    ...
                    alert_msg = (
                        ...
                        f"🪙 **คู่เงิน:** `{name}`\n"
                        f"⏰ **เวลาเข้าซื้อ:** `{target_str}` น. (เตรียมตัวล่วงหน้า 2 นาที)\n"
                    ...
                    while datetime.now() < entry_time:
                        remaining_seconds = int((entry_time - datetime.now()).total_seconds())
                        if remaining_seconds == 10:
                            print(f"⚠️ [{datetime.now().strftime('%H:%M:%S')}] เหลือ 10 วินาที! เตรียมกดออร์เดอร์ {target_str} น. -> {action_text}")
                            time.sleep(1)
                        time.sleep(0.1)
...
def run_strategy_sniper_5m():
    ...
    while True:
        try:
            now = datetime.now()
            current_min, current_sec = now.minute, now.second
            min_to_next = 1 - (current_min % 1) if current_min % 1 != 0 else 1
            if current_sec == 0: min_to_next = 0
                
            target_time = now + timedelta(minutes=min_to_next, seconds=-current_sec, microseconds=-now.microsecond)
            warning_time = target_time - timedelta(seconds=45)
...
def analyze_strategy3(df_1m, df_5m, df_15m, now_utc):
    valid, _ = check_time_filters(now_utc)
...
def run_strategy_3_multitimeframe():
    ...
    while True:
        try:
            now = datetime.now()
            current_min, current_sec = now.minute, now.second
            min_to_next = 5 - (current_min % 5) if current_min % 5 != 0 else 5
            if min_to_next == 5 and current_min % 5 == 0 and current_sec == 0: min_to_next = 0
                
            target_time = now + timedelta(minutes=min_to_next, seconds=-current_sec, microseconds=-now.microsecond)
            warning_time = target_time - timedelta(minutes=2, seconds=30)
...
# ==========================================
# ⚡ สูตรที่ 4: SNIPER PRO SOL/USD (แจ้งเตือนเตรียมตัว 2 นาที)
# ==========================================
...
def pair_worker_sol(pair):
    ...
    while True:
        try:
            ...
            now = datetime.now()
            entry_time = now.replace(second=0, microsecond=0) + timedelta(minutes=2)
            action_icon = "🟢 ซื้อขึ้น (CALL)" if action == "CALL" else "🔴 ซื้อลง (PUT)"
            
            print(f"\n--------------------------------------------------")
            print(f"{color}🎯 [SNIPER PRO SOL/USD] พบสัญญาณเกรด [{grade}] ทรงพลัง! [{name}]{color}")
            print(f"⏰ เวลาเตรียมตัว: 2 นาที | จุดเข้าซื้อเวลา: {entry_time.strftime('%H:%M:%S')} น. | ไม้ที่ {step}/3")
            print(f"--------------------------------------------------")
            
            discord_msg = (
                f"⚡ **[สูตร 4: Sniper Pro SOL/USD - เตือนเตรียมตัวล่วงหน้า 2 นาที!]**\n"
                ...
                f"📊 **รายละเอียด:** {reason}\n"
                f"⏳ **สถานะ:** กรุณาเตรียมเปิดออเดอร์ตามเวลาที่ระบุข้างต้น"
            )
...
# ==========================================
# 🚀 สูตรที่ 6: MOMENTUM & VOLATILITY PRO (1M Breakout & Reversal)
# ==========================================
...
def start_bot_system():
    print("==================================================")
    print("🚀 ระบบ Multi-Strategy Bot (สูตรที่ 1 ถึง สูตรที่ 7 A+) พร้อมใช้งานเต็มรูปแบบ")
    print("==================================================")
    
    # แสดงสถิติสะสมจากฐานข้อมูล SQLite ทันทีเมื่อเริ่มต้นโปรแกรม
    print_total_system_stats_summary()
    
    send_discord_alert(
        "🚀 **Bot Online (24/7 Mode)**: เปิดใช้งานระบบเทรดอัตโนมัติครบถ้วนเรียบร้อยแล้ว!\n"
        "- 🎯 **สูตร 1**: สูตร 6 คู่เงิน 1m (EUR/USD, GBP/USD, AUD/USD, USD/JPY, EUR/GBP, GBP/JPY)\n"
        "- 🧠 **สูตร 2**: SMC 1M\n"
        "- 📐 **สูตร 3**: Institutional MTF\n"
        "- ⚡ **สูตร 4**: Sniper Pro SOL/USD (แจ้งเตือนล่วงหน้า 2 นาที)\n"
        "- 🇬🇧🇯🇵 **สูตร GBP/JPY**: 8 Indicators + เตือน Discord ล่วงหน้า 2 นาที\n"
        "- 🚀 **สูตร 6**: Momentum & Volatility Pro 1M\n"
        "- 🔥 **สูตรที่7 A+**: EUR/JPY Confidence Score System (เตือน Discord ล่วงหน้า 2 นาที)"
    )
    
    # รัน Thread แยกสำหรับการวิเคราะห์ต่างๆ
