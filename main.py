...
# สร้างไฟล์ SIGZY BRAIN V6.0 ที่แก้ไขแล้ว
code = r'''import time, json, os, urllib.request
...
# ถ้าไม่มี Gemini ให้ใช้ Bot อย่างเดียว
STRICT_AI_CONSENSUS = bool(GEMINI_API_KEY)
...
for s in FOREX:
    TUNING[s] = {
        "sr": .005 if s in ("GBPUSD", "USDJPY", "EURUSD", "AUDUSD",
                            "NZDUSD", "USDCAD", "USDCHF", "EURGBP") else .006,
        "zone": .012 if s in ("GBPUSD", "USDJPY", "EURUSD", "AUDUSD",
                              "NZDUSD", "USDCAD", "USDCHF", "EURGBP") else .013,
        "vol": 1.20
    }
...
def gemini_call(role, data):
    if not GEMINI_API_KEY:
        return False, "NO_KEY", 0.0, "Gemini key not configured"
    ...
    prompt = {
        ...
        "rules": [
            ...
            "Do not invent candles or prices.",
    ...
    try:
        ...
        confidence = max(
            0.0,
            min(100.0, float(j.get("confidence", 0)))
        )

        reason = str(j.get("reason", "No reason"))
...
def ai_final_check(symbol, action, setup, trend, candles,
                   bot_score, pair_hist, setup_hist):

    if not GEMINI_API_KEY:
        return {
            "approved": True,
            "status": "BOT_ONLY",
            "final_score": bot_score,
            "ai1": {
                "decision": "BYPASS",
                "confidence": 100.0,
    ...
    base = {
        ...
        "trend": trend,
        "bot_score": bot_score,
        "pair_history": pair_hist,
        "setup_history": setup_hist,
        "candles": candles_for_ai(candles)
    }

    ok1, d1, c1, r1 = gemini_call(
        "AI #1 Pattern Analyst. Inspect candle structure, wicks, bodies, "
        "momentum, support/resistance and trend alignment. Approve only "
        "when the supplied evidence supports the proposed direction.",
        base
    )

    print(
        f"🤖 AI1 {symbol}: {d1} {c1:.1f}% | {r1}",
        flush=True
    )
    ...
    ok2, d2, c2, r2 = gemini_call(
        "AI #2 Risk/Contrarian Analyst. Try to prove the proposed trade "
        "wrong. Look for trend conflict, fake reversal, late entry, weak "
        "rejection, abnormal momentum and hidden risk. Do not approve "
        "merely because AI1 approved.",
        data
    )

    print(
        f"🛡️ AI2 {symbol}: {d2} {c2:.1f}% | {r2}",
        flush=True
    )
    ...
    if STRICT_AI_CONSENSUS:
        approved = (
            d1 == "APPROVE"
            and d2 == "APPROVE"
            and c1 >= AI_CONFIDENCE_THRESHOLD
            and c2 >= AI_CONFIDENCE_THRESHOLD
            and final >= FINAL_SCORE_THRESHOLD
        )
    else:
        approved = (
            final >= FINAL_SCORE_THRESHOLD
            and (
                (d1 == "APPROVE" and c1 >= AI_CONFIDENCE_THRESHOLD)
                or
...
def init_memory():
    ...
    try:
        ...
        m.setdefault("version", "SIGZY_V6")
        m.setdefault("stats", {})
        m.setdefault("score_bands", {})
        m.setdefault("ai_stats", {})
        m.setdefault("setups", {})
        ...
        for k, v in base["ai_stats"].items():
            m["ai_stats"].setdefault(k, v)

        # รองรับ memory เก่าที่ใช้ชื่อ losss
        old_loss = int(m["stats"].pop("losss", 0))
        if old_loss:
            m["stats"]["losses"] += old_loss
...
def analyze(c, m, symbol):
    ...
    trend = (
        "BULL_STRONG" if cur["close"] > e20 > e50 > e100 else
        "BEAR_STRONG" if cur["close"] < e20 < e50 < e100 else
        "BULL" if cur["close"] > e50 else
        "BEAR" if cur["close"] < e50 else
        "RANGE"
    )
    ...
    cb, ct = historical_bonus(m, symbol, "CALL", call_setup)
    pb, pt = historical_bonus(m, symbol, "PUT", put_setup)
    ...
    cv = (
        (
            ns and bull and (bp or be) and bf
            and trend != "BEAR_STRONG"
        )
        or
        (
            trend in ("BULL", "BULL_STRONG")
            and bull and bf and hv and p > e20 and not nr
        )
    )

    pv = (
        (
            nr and bear and (sp or se) and sf
            and trend != "BULL_STRONG"
        )
        or
        (
            trend in ("BEAR", "BEAR_STRONG")
            and bear and sf and hv and p < e20 and not ns
        )
    )

    if cm["decided"] >= SETUP_MIN_HISTORY and cm["wr"] < 55:
        cv = False

    if pm["decided"] >= SETUP_MIN_HISTORY and pm["wr"] < 55:
        pv = False
    ...
    if cv and cs >= MIN_SCORE_THRESHOLD and cs > ps:
        action, score, setup, wr, tag = (
            "CALL", cs, call_setup, cm["wr"], ct
        )
    elif pv and ps >= MIN_SCORE_THRESHOLD and ps > cs:
        action, score, setup, wr, tag = (
            "PUT", ps, put_setup, pm["wr"], pt
        )
...
def rank_candidates(candidates, m):
    def key(x):
        ...
        ps = pair_stats(m, s)
        q = quality(m, s, a["action"], a["setup"])

        pwr = (
            ps["wr"]
            if ps["decided"] >= PAIR_MIN_HISTORY and ps["wr"] is not None
            else 50
        )

        swr = (
            q["wr"]
            if q["decided"] >= SETUP_MIN_HISTORY and q["wr"] is not None
            else 50
        )
...
def record(m, tr, result):
    ...
    if ai.get("ai1_decision") == "APPROVE":
        ai_stats["ai1_approvals"] += 1
    elif ai.get("ai1_decision") == "REJECT":
        ai_stats["ai1_rejections"] += 1

    if ai.get("ai2_decision") == "APPROVE":
        ai_stats["ai2_approvals"] += 1
    elif ai.get("ai2_decision") == "REJECT":
        ai_stats["ai2_rejections"] += 1

    if (
        ai.get("ai1_decision") == "APPROVE"
        and ai.get("ai2_decision") == "APPROVE"
    ):
        ai_stats["consensus_approved"] += 1

    update_band(m, tr.get("entry_score", 0), result)
    save(m)

    d = s["wins"] + s["losses"]
    wr = s["wins"] / d * 100 if d else 0

    msg = (
        f"📊 PAPER RESULT V6.0\n"
        f"ตลาด: {tr['symbol']}\n"
        f"ผลล่าสุด OPP3: {result}\n"
        f"Action: {tr['action']}\n"
        f"Setup: {tr['setup']}\n"
        f"Bot Score: {tr['entry_score']:.1f}\n"
        f"Final AI Score: {tr.get('final_ai_score', 0):.1f}\n"
        f"AI1: {ai.get('ai1_decision')} "
        f"{ai.get('ai1_confidence', 0):.1f}%\n"
        f"AI2: {ai.get('ai2_decision')} "
        f"{ai.get('ai2_confidence', 0):.1f}%\n"
        f"OPP1={tr.get('opportunity_results', {}).get('1')} | "
        f"OPP2={tr.get('opportunity_results', {}).get('2')} | "
        f"OPP3={tr.get('opportunity_results', {}).get('3')}\n"
        f"Setup WR: {wr:.1f}%\n"
        f"รวม: WIN {st['wins']} | DRAW {st['draws']} | "
        f"LOSS {st['losses']}\n"
        f"🧠 Memory: {key}"
    )
...
def run():
    ...
    print("🧠 BOT + 🤖 AI1 PATTERN + 🛡️ AI2 RISK")
    print("🏆 BEST PAIR SELECTOR + 📚 REAL MEMORY")
    print("🇹🇭 THAILAND UTC+7")
    print("=" * 72)

    print(
        f"AI Model: {GEMINI_MODEL} | "
        f"AI Threshold: {AI_CONFIDENCE_THRESHOLD} | "
        f"Final: {FINAL_SCORE_THRESHOLD} | "
        f"Strict: {STRICT_AI_CONSENSUS}",
        flush=True
    )
    ...
    if DISCORD_WEBHOOK_URL:
        ...
        discord(
            "🟢 SIGZY BRAIN V6.0 ONLINE\n"
            "🧠 Bot + 🤖 AI1 Pattern + 🛡️ AI2 Risk\n"
            "🏆 Best Pair Selector + 📚 Real Memory\n"
            "🇹🇭 Thailand UTC+7"
        )
    else:
        print("⚠️ Discord webhook not set.", flush=True)

    if not GEMINI_API_KEY:
        print(
            "⚠️ Gemini key missing: Bot-only mode.",
            flush=True
        )
    ...
    while True:
        ...
        print(
            "\n" + "=" * 72 +
            f"\n🧠 SCAN {now.strftime('%Y-%m-%d %H:%M:%S')} TH",
            flush=True
        )

        # ---------- UPDATE ACTIVE PAPER TRADES ----------
        done = []
        ...
        for symbol in SYMBOLS[:MAX_SCAN_PAIRS]:
            ...
            if status == "BLOCK" or symbol in active:
                continue

            if (
                symbol in last_alert
                and (now - last_alert[symbol]).total_seconds()
        ...
        for cand in selected:
            ...
            tr = {
                ...
                "entry_score": a["score"],
                "final_ai_score": ai["final_score"],
                "setup": a["setup"],
                "opportunity": 0,

                # สำคัญ: ใช้ timestamp ของแท่งเข้า
                # เพื่อไม่ให้นับแท่งเข้าเป็น OPP1
                "last_ts": cand["timestamp"],

                "opportunity_results": {},

                "ai": {
                    "ai1_decision": ai["ai1"]["decision"],
                    "ai1_confidence": ai["ai1"]["confidence"],
                    "ai1_reason": ai["ai1"]["reason"],
                    "ai2_decision": ai["ai2"]["decision"],
                    "ai2_confidence": ai["ai2"]["confidence"],
                    "ai2_reason": ai["ai2"]["reason"]
                }
            }
            ...
            msg = (
                ...
                f"🤖 AI1 Pattern: "
                f"{ai['ai1']['decision']} "
                f"{ai['ai1']['confidence']:.1f}%\n"
                f"🛡️ AI2 Risk: "
                f"{ai['ai2']['decision']} "
                f"{ai['ai2']['confidence']:.1f}%\n"
                f"🏆 FINAL SCORE: "
                f"{ai['final_score']:.1f}/100\n\n"
                f"Setup: {a['setup']}\n"
                f"Trend: {a['trend']}\n"
                ...
                f"🤖 AI1: {ai['ai1']['reason']}\n"
                f"🛡️ AI2: {ai['ai2']['reason']}\n\n"
                f"📚 Paper Track: OPP1 ➔ OPP2 ➔ OPP3\n"
                f"🧠 Real Memory Learning: ON\n"
                f"🇹🇭 Thailand UTC+7\n"
        ...
        st = m["stats"]
        d = st["wins"] + st["losses"]
        wr = st["wins"] / d * 100 if d else 0
        ai = m["ai_stats"]

        print(
            f"📊 MEMORY: {st['total_setups']} setups | "
            f"WIN={st['wins']} LOSS={st['losses']} "
            f"DRAW={st['draws']} | WR={wr:.2f}% | "
            f"Active={len(active)}",
            flush=True
        )
        ...
        print(
            f"🤖 AI: "
            f"A1 {ai['ai1_approvals']}A/"
            f"{ai['ai1_rejections']}R | "
            f"A2 {ai['ai2_approvals']}A/"
            f"{ai['ai2_rejections']}R | "
            f"Consensus={ai['consensus_approved']} | "
            f"Errors={ai['ai_errors']}",
            flush=True
        )
...
if __name__ == "__main__":
    try:
        ...
        print("🛑 SIGZY BRAIN V6.0 stopped.")
    except Exception as e:
...
print("แก้จุดสำคัญแล้ว: JSON AI, Memory เก่า, OPP1→2→3 และการไม่นับแท่งเข้าเป็น OPP1")
