...
code = '''import os, json, time, sqlite3, threading, urllib.request, urllib.parse
from datetime import datetime
from zoneinfo import ZoneInfo
import numpy as np

# ============================================================
# V15 ADAPTIVE PAPER TRAINER - FIXED
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "V15-Paper-Trainer/Fixed"
            },
            method="POST"
        )
...
def fetch(symbol, interval="1m", limit=250):
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={"User-Agent": "V15-Paper-Trainer/Fixed"}
        )
...
def atr_pct(p, n=14):
    ...
    for i in range(1, len(closed)):
        h, l, pc = closed[i]["h"], closed[i]["l"], closed[i-1]["c"]
...
def analyze(symbol, name):
    ...
    if len(k) < 220:
        return {
            "name": name, "symbol": symbol, "decision": "NO TRADE",
            "grade": "C", "score": 0, "trap": 100,
            "regime": "UNKNOWN", "mtf": ("NEUTRAL",)*3,
            "entry_time": 0, "price": 0, "flags": ["DATA<220"]
        }
    ...
    sweep_call = cur["l"] < support and cur["c"] > support and cur["dn_wick"] >= 28 and cur["green"]
    sweep_put = cur["h"] > resistance and cur["c"] < resistance and cur["up_wick"] >= 28 and cur["red"]
    ...
    if action == "CALL":
        if h == "BULLISH": score += 10
        if m15 == "BULLISH": score += 10
        if m5 == "BULLISH": score += 5
        if h == "BEARISH":
            trap += 45
        ...
        if h == "BEARISH": score += 10
        if m15 == "BEARISH": score += 10
        if m5 == "BEARISH": score += 5
        if h == "BULLISH":
            trap += 45
    ...
    if action == "CALL" and resistance > price:
        if (resistance-price)/price*100 < atr*.45:
            trap += 65
            flags.append("CALL near resistance")
    if action == "PUT" and support < price:
        if (price-support)/price*100 < atr*.45:
            trap += 65
            flags.append("PUT near support")

    aligned = (
        action == "CALL" and h == "BULLISH" and m15 == "BULLISH"
    ) or (
        action == "PUT" and h == "BEARISH" and m15 == "BEARISH"
    )
    ...
    return {
        "name": name, "symbol": symbol, "decision": decision,
        "grade": grade, "score": score, "trap": min(trap,100),
        "regime": regime, "mtf": bias, "entry_time": entry_time,
        "price": price, "rsi": rr, "flags": flags
    }
...
class Journal:
    def __init__(self):
        ...
        with sqlite3.connect(self.path) as c:
            c.execute("""CREATE TABLE IF NOT EXISTS paper(
                ...
                ts TEXT, pair TEXT, action TEXT, entry REAL,
                entry_time INTEGER, score REAL, grade TEXT,
                trap REAL, source TEXT, result TEXT DEFAULT 'PENDING',
                exit REAL, pnl REAL)""")

    def add(self, r, action, source):
        with self.lock, sqlite3.connect(self.path) as c:
            cur = c.execute("""INSERT INTO paper
                (ts,pair,action,entry,entry_time,score,grade,trap,source)
                VALUES(?,?,?,?,?,?,?,?,?)""", (
                    thai_text(), r["name"], action, r["price"],
                    r["entry_time"], r["score"], r["grade"],
                    r["trap"], source
            ))
    ...
    def finish(self, sid, result, exit_price, pnl):
        with self.lock, sqlite3.connect(self.path) as c:
            c.execute("""UPDATE paper SET result=?,exit=?,pnl=? WHERE id=?""",
...
class Paper:
    ...
    def best(self):
        ...
        for name, symbol in PAIRS:
            ...
            try:
                ...
                # Real V15 signal gets priority.
                real = r["decision"] in ("CALL","PUT")
                if real:
                    direction = r["decision"]
                else:
                    # FORCE PAPER: choose direction from strongest MTF bias.
                    h,m15,m5 = r["mtf"]
                ...
                rank = r["score"] + (15 if real else 0) - r["trap"]*.25
                rows.append((rank,r,direction,real))
                print(f"[CANDIDATE] {name} {direction} "
                      f"grade={r['grade']} score={r['score']:.1f} "
                      f"trap={r['trap']:.1f} rank={rank:.1f}")
            except Exception as e:
                print("[ANALYZE ERROR]", name, e)
    ...
    def open_cycle(self):
        ...
        order = {
            "id":pid, "sid":sid, "name":r["name"],
            "symbol":r["symbol"], "action":direction,
            "entry":price, "entry_time":entry_time,
            "score":r["score"], "trap":r["trap"],
    ...
    def resolve(self):
        ...
        for o in orders:
            ...
            # Entry is a closed candle. Evaluate the next closed 1m
            # candle at +1m; with the 2-minute horizon this waits
            # until the second candle after entry has closed.
            ...
            discord(
                ...
                f"L `{self.session['LOSS']}` / D `{self.session['DRAW']}`\\n"
                f"🎯 Session WR `{wr:.1f}%`\\n"
                f"🇹🇭 `{thai_text()}`"
            )
    ...
    while True:
        try:
            ...
            if current != last:
                ...
                print(
                    f"[MASTER] {name} "
                    f"{r['decision']} grade={r['grade']} "
                    f"score={r['score']:.1f} trap={r['trap']:.1f}"
                )
...
def start():
    print("="*60)
    print("🚀 V15 ADAPTIVE PAPER TRAINING - FIXED")
    print("="*60)
    ...
    if discord_enabled():
        ...
        print("[DISCORD START]", ok)
    else:
        print("⚠️ DISCORD WEBHOOK NOT CONFIGURED")
        print("Set DISCORD_WEBHOOK_URL in Railway Variables.")
...
path = Path("/mnt/data/v15_adaptive_paper_training_FIXED.py")
