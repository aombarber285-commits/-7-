...
def analyze_15m_opportunity(symbol, candles):
    ...
    if is_strong_bull or is_hammer or is_bull_engulfing:
        confirmations_call += 1
        if is_bull_engulfing: reasons.append("Bullish Engulfing")
        elif is_hammer: reasons.append("Hammer PinBar")
        else: reasons.append("Strong Bull Candle")

    if is_strong_bear or is_shooting_star or is_bear_engulfing:
        confirmations_put += 1
        if is_bear_engulfing: reasons.append("Bearish Engulfing")
        elif is_shooting_star: reasons.append("Shooting Star")
        else: reasons.append("Strong Bear Candle")
    ...
    if abs(price - support_level) <= (support_level * 0.0008) and above_ema:
        confirmations_call += 1
        reasons.append("Support Zone Confluence")

    if abs(price - resistance_level) <= (resistance_level * 0.0008) and below_ema:
        confirmations_put += 1
        reasons.append("Resistance Zone Confluence")
...
def scan_all_symbols():
    ...
    print(f"\n[{now_text()}] 🔍 สแกน 8 คู่เงิน (yfinance)...")
...
def main():
    print("SIGZY AI 15M - YFINANCE TESTER RUNNING")
    send_discord("📱 **SIGZY YFINANCE TESTER ONLINE**\nเริ่มทดสอบผ่าน yfinance ฟรี ไม่ติด API Limit!")
