...
STRICT_AI_CONSENSUS=os.getenv("STRICT_AI_CONSENSUS","true").lower() in ("1","true","yes","on")
...
def discord(msg):
    if not DISCORD_WEBHOOK_URL:
        print("[WARN] DISCORD_WEBHOOK_URL is not set.",flush=True); return False
    try:
...
def ai_final_check(symbol,action,setup,trend,candles,bot_score,pair_hist,setup_hist):
    ...
    ok1,d1,c1,r1=gemini_call(
        ...
        base)
    print(f"ð¤ AI1 {symbol}: {d1} {c1:.1f}% | {r1}",flush=True)
    if not ok1:
    ...
    ok2,d2,c2,r2=gemini_call(
        ...
        data)
    print(f"ð¡ï¸ AI2 {symbol}: {d2} {c2:.1f}% | {r2}",flush=True)
    if not ok2:
...
def record(m,tr,result):
    ...
    msg=(f"ð PAPER RESULT V6.0\nà¸à¸¥à¸²à¸: {tr['symbol']}\nà¸à¸¥à¸¥à¹à¸²à¸ªà¸¸à¸: {result}\n"
         ...
         f"à¸£à¸°à¸à¸à¸£à¸§à¸¡: à¸à¸à¸° {st['wins']} | à¹à¸ªà¸¡à¸­ {st['draws']} | à¹à¸à¹ {st['losses']}\nð§  Memory updated: {key}")
    print(msg,flush=True);discord(msg)

# ---------------- MAIN ----------------

def run():
    print("="*72);print("ð¤ SIGZY BRAIN V6.0");print("ð§  BOT + ð¤ AI1 PATTERN + ð¡ï¸ AI2 RISK")
    print("ð BEST PAIR SELECTOR + ð REAL MEMORY");print("="*72)
    ...
    if DISCORD_WEBHOOK_URL:
        ...
        discord("ð¢ SIGZY BRAIN V6.0 ONLINE\nð§  Bot + ð¤ AI1 Pattern + ð¡ï¸ AI2 Risk\nð Best Pair Selector + ð Real Memory")
    else:print("â ï¸ DISCORD_WEBHOOK_URL is not set.",flush=True)
    if not GEMINI_API_KEY:print("â GEMINI_API_KEY missing: AI-approved signals are disabled.",flush=True)
    m=init_memory();active={};last_alert={};last_signal=datetime.now()

    while True:
        now=datetime.now();print("\n"+"="*72);print("ð§  SCAN",now.strftime("%Y-%m-%d %H:%M:%S"),flush=True)
        ...
        for symbol,tr in list(active.items()):
            ...
            print(f"ð {symbol} | OPP {opp}/3 | {tr['action']} | {result}",flush=True)
        ...
        for symbol in SYMBOLS[:MAX_SCAN_PAIRS]:
            ...
            if a["action"]!="WAIT" and a["score"]>=MIN_SCORE_THRESHOLD:
                ...
                print(f"ð¯ CANDIDATE {symbol} {a['action']} {a['score']:.1f} {a['setup']} | Pair={status} | PairWR={ps['wr'] if ps['wr'] is not None else 'N/A'}",flush=True)
        ...
        for cand in ranked[:5]:
            ...
            if ai["approved"]:approved.append(cand);print(f"â CONSENSUS {symbol} Final={ai['final_score']:.1f}",flush=True)
            else:print(f"â REJECTED {symbol} Final={ai['final_score']:.1f}",flush=True)
        ...
        for cand in selected:
            ...
            ps=pair_stats(m,symbol);wr=f"{a['real_wr']:.1f}%" if a["real_wr"] is not None else "à¸à¸³à¸¥à¸±à¸à¹à¸à¹à¸à¸ªà¸à¸´à¸à¸´"
            pwr=f"{ps['wr']:.1f}% ({ps['decided']} decided)" if ps["wr"] is not None else "à¸à¸³à¸¥à¸±à¸à¹à¸£à¸µà¸¢à¸à¸£à¸¹à¹"
            msg=(f"ð¨ SIGZY BRAIN V6.0\nð BEST PAIR OF THIS SCAN\n\nà¸à¸¥à¸²à¸: {symbol}\n"
                 f"à¹à¸§à¸¥à¸²: {now.strftime('%H:%M:%S')}\nà¸£à¸²à¸à¸²: {cand['price']}\n"
                 f"à¸à¸´à¸¨à¸à¸²à¸: {'CALL ð¢' if a['action']=='CALL' else 'PUT ð´'}\n\n"
                 f"ð§  BOT SCORE: {a['score']:.1f}/100\n"
                 f"ð¤ AI #1 Pattern: {ai['ai1']['decision']} {ai['ai1']['confidence']:.1f}%\n"
                 f"ð¡ï¸ AI #2 Risk: {ai['ai2']['decision']} {ai['ai2']['confidence']:.1f}%\n"
                 f"ð FINAL SCORE: {ai['final_score']:.1f}/100\n\n"
                 f"Setup: {a['setup']}\nTrend: {a['trend']}\nHistorical Setup WR: {wr}\nPair WR: {pwr}\n"
                 f"Memory: {a.get('memory_tag','LEARNING')}\n\n"
                 f"ð¤ AI1: {ai['ai1']['reason']}\nð¡ï¸ AI2: {ai['ai2']['reason']}\n\n"
                 f"ð Paper Track: Opportunity 1 â 2 â 3\nð§  Real Memory Learning: ON\n"
                 f"â ï¸ Paper Trade â à¹à¸¡à¹à¹à¸à¹à¸à¸³à¸ªà¸±à¹à¸à¸à¸·à¹à¸­à¸à¸²à¸¢à¸­à¸±à¸à¹à¸à¸¡à¸±à¸à¸´")
            print("\n"+msg,flush=True);discord(msg)

        st=m["stats"];d=st["wins"]+st["losses"];wr=st["wins"]/d*100 if d else 0;ai=m["ai_stats"]
        print(f"ð MEMORY: {st['total_setups']} setups | WIN={st['wins']} LOSS={st['losses']} DRAW={st['draws']} | WR={wr:.2f}% | Active={len(active)}",flush=True)
        print(f"ð¤ AI: A1 {ai['ai1_approvals']}A/{ai['ai1_rejections']}R | A2 {ai['ai2_approvals']}A/{ai['ai2_rejections']}R | Consensus={ai['consensus_approved']}",flush=True)
        print(f"â³ Wait {PAPER_INTERVAL_SECONDS}s...",flush=True)
        time.sleep(PAPER_INTERVAL_SECONDS)

if __name__=="__main__":
    try:run()
    except KeyboardInterrupt:print("ð SIGZY BRAIN V6.0 stopped.")
